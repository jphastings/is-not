...
const stubFetch = (async (url: string | URL | Request) => {
  ...
  if (u.startsWith('https://pds.example/xrpc/com.atproto.repo.getRecord?')) {
    ...
    expect([q.get('repo'), q.get('collection'), q.get('rkey')]).toEqual([
      'did:plc:example',
      'com.example.thing',
      '3abc',
    ]);
    return Response.json({
      uri,
      cid,
      value: { $type: 'com.example.thing', name: 'A Thing', externalId: '42' },
    });
  }
...
describe('fetchRecord', () => {
  it('resolves the PDS and fetches the record', async () => {
    await expect(fetchRecord(uri, stubFetch)).resolves.toEqual({
      cid,
      record: { $type: 'com.example.thing', name: 'A Thing', externalId: '42' },
    });
  });
