...
Second piece of the isnot.at appview (bean ISNOT-0gxo). Extends the `at.isnot.review` lexicon
so a review carries enough about its subject to display, and teaches the ingester to record
...
`lexicons/at/isnot/review.json` gains two defs and the record's `subject` becomes a ref to
...
| | cid | string, format cid | required. The uri's cid when the review was last updated |
...
imdbId or isbn13". `tags`, `createdAt` and `updatedAt` are unchanged.
...
`review_tags` and `review_identifiers` hold one row per tag/identifier and are replaced
wholesale on each review upsert (a repeated adjective in one record keeps the last entry);
rows cascade away when the review is deleted. An empty `handle` means unresolved.
...
| event | action |
|---|---|
| commit create / update, valid record | upsert the review; ensure an accounts row exists for the DID |
...
| account `deleted`, sync | unchanged: delete the DID's reviews. The accounts row stays |
...
asserts title and type on the review row and identifiers via the `review_identifiers` table; a record missing `title` is invalid; a
